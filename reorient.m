function [nodes_final, coords_final, coords_final_unit, Temp_Coordinates_Unit, M1Pro_final, DMAA_final, NC_nav_final, NC_cub_final, MLCR_final, HAA_final, TTA_final, talus_coords_FAO, z_min_xyz_final, z_min_xyz_MSA_final, MEARY_final, MDTA_final, TLSA_final] = reorient(Temp_Nodes_Coords,cm_nodes,side_indx,RTs)
...
Temp_Coordinates_origin = Temp_Nodes_Coords(end-25,:); % same as **
Temp_Coordinates_temp = Temp_Nodes_Coords(end-25:end-20,:) - Temp_Coordinates_origin; %% second end should be same as ^^ below and then add 5 for the first end **
...
elseif isempty(RTs.sR_calcaneus) == 0
    nodes_coords_final_i4 = ((RTs.sR_calcaneus)\(Temp_Nodes_Coords'))';
...
M1Pro_final = nodes_coords_final(end-19:end-18,:);
...
HAA_final = nodes_coords_final(end-12:end-11,:);
TTA_final = nodes_coords_final(end-10:end-9,:);
talus_coords_FAO = nodes_coords_final(end-8,:);
z_min_xyz_final = nodes_coords_final(end-7,:);
z_min_xyz_MSA_final = nodes_coords_final(end-6,:);
MEARY_final = nodes_coords_final(end-5:end-4,:);
MDTA_final = nodes_coords_final(end-3:end-2,:);
TLSA_final = nodes_coords_final(end-1:end,:);

nodes_coords_final = nodes_coords_final(1:end-20,:); % add 1 to largest 'final' ^^
