function [TR, TT, ER, t] = icp_constrained(q,p,varargin)
...
inp.addParamValue('MaxRotationDeg', 60, @(x)isnumeric(x) && isscalar(x) && x > 0 && x <= 60)
...
for k=1:arg.iter
    ...
    % Constrain Euler angles to maximum per-axis rotation
    eul = rotm2eul(R);  % [yaw, pitch, roll] in radians
    max_angle = deg2rad(arg.MaxRotationDeg);  % scalar threshold

    % Clamp each angle component
    eul_clamped = max(min(eul, max_angle), -max_angle);

    % If any clamping occurred, reconstruct R
    if any(abs(eul) > max_angle)
        if arg.Verbose
            fprintf('Clamping Euler angles: original = [%0.2f, %0.2f, %0.2f], clamped = [%0.2f, %0.2f, %0.2f]\n', ...
                rad2deg(eul), rad2deg(eul_clamped));
        end
        R = eul2rotm(eul_clamped, 'ZYX');
    end
